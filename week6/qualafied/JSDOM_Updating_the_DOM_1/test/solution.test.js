...
describe("Updating the DOM", () => {
  ...
  it("should have 8 h2 headings in 8 article elements", async () => {
    const count = await page.evaluate(() => document.querySelectorAll('article h2').length);
    expect(count).toEqual(8);
  });
  ...
  it("should have 3 elements with tutorial class", async () => {
    const count = await page.evaluate(() => document.querySelectorAll('.tutorial').length);
    expect(count).toEqual(3);
  });
  
  it("should have 5 elements in the article  section", async () => {
    const count = await page.evaluate(() => document.querySelectorAll('.articles article').length);
    expect(count).toEqual(5);
  });
  
  it("should have 3 elements in the tutorials section", async () => {
    const count = await page.evaluate(() => document.querySelectorAll('.tutorials article').length);
    expect(count).toEqual(3);
  });
});