...
function render(contacts) {
  ...
  for (let contact of contacts) {
    contactContainer.innerHTML += renderContact(contact);
  }
}
...
function filterHandler() {
  const filterMenu = document.querySelector("#filterOptions");
  filterMenu.addEventListener("change", (event) => {
    const value = event.target.value;
    if (value == 0) {
      render(window.contacts);
    } else {
      const filtered = filterByCity(value);
      render(filtered);
    }
  });
}
function getCities(contacts) {
  const cities = [];
  for (let contact of contacts) {
...
function loadCities(contacts) {
  if (contacts.length != 0) {
    const cityArray = getCities(contacts);
    const filterContainer = document.querySelector("#filterOptions");
    filterContainer.innerHTML = `<option value="0">-- Select a city --</option>`;
    for (let city of cityArray) {
      filterContainer.innerHTML += `<option value = "${city}">${city}</option>`;
    }
  } else {
    const filterContainer = document.querySelector("#filterOptions");
    filterContainer.innerHTML = `<option value="0">-- Select a city --</option>`;
...
function deleteButtonHandler() {
  ...
  main.addEventListener("click", (event) => {
    if (event.target.nodeName == "BUTTON") {
      ...
      const contacts = deleteContact(contactContainer.dataset.id);
      render(contacts);
      loadCities(contacts);
    }
  });
