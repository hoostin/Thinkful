...
describe("Index Page", () => {
  ...
  it("should have 8 articles", async () => {
    const number = await page.evaluate(() => document.querySelectorAll(".article").length)
    expect(number).toEqual(8);
  });
  
  it("should have 8 expand buttons", async () => {
    const number = await page.evaluate(() => document.querySelectorAll(".expand_button").length)
    expect(number).toEqual(8);
  });
  
  it("should have 8 highlight buttons", async () => {
    const number = await page.evaluate(() => document.querySelectorAll(".highlightBtn").length)
    expect(number).toEqual(8);
  });
});
...
describe("expandArticleBody()", () => {
  ...
  it("should expand the right article", async () => {
    await page.click('.expand_button');
    const display = await page.evaluate(() => document.querySelector('.article_body').style.display);
  ...
  it("should change symbol on button", async () => {
    await page.click('.expand_button');
    const display = await page.evaluate(() => document.querySelector('.expand_button').innerText);
...
describe("highlightArticle()", () => {
  ...
  it("should be able to highlight a single article", async () => {
   ...
   const count = await page.evaluate(() => document.querySelectorAll('.highlight').length);
   expect(count).toEqual(1);
  });
  ...
  it("should change symbol on button", async () => {
    await page.click('.expand_button');
     await page.click('.highlightBtn');
    const display = await page.evaluate(() => document.querySelector('.highlightBtn').innerText);
