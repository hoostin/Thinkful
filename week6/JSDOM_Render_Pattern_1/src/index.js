...
function render() {
  const bookArray = window.books;
  const bookContainer = document.querySelector("#cartItems");
  ...
  if (bookArray.length != 0) {
    ...
    bookContainer.innerHTML = "Nothing in cart";
  }
  const total = calculateTotal();
  const totalContainer = document.querySelector("#cartTotal .total-price");
...
function sortByPrice() {
  const sorted = window.books.sort((book1, book2) => book1.price - book2.price);
  render();
  return sorted;
}
