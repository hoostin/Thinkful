...
class ParkingLot {
	...
	enter(licensePlateNumber) {
		if (this.vacantSpaces) {
			const available = this.spaces.findIndex((spot) => spot === "vacant");
			if (available !== -1) {
				this.spaces[available] = licensePlateNumber;
			}
		} else {
	...
	leave(licensePlateNumber) {
		const current = this.spaces.findIndex(
			(spot) => spot === licensePlateNumber
		);
		if (current != -1) {
			this.revenue += this.rate;
			if (this.queue.first) {
				const nextCar = this.queue.dequeue();
				this.spaces[current] = nextCar;
			} else {
				this.spaces[current] = "vacant";
			}
		} else {
