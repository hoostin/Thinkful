class BinarySearchTree {
	...
	countLeavesRecurse(node) {
		if (node == null) return 0;
		if (node.left == null && node.right == null) {
			return 1;
		} else {
			return (
				this.countLeavesRecurse(node.left) + this.countLeavesRecurse(node.right)
			);
		}
	}
	countLeaves(count = 0) {
		...
		return (count = this.countLeavesRecurse(this));
	}
	checkHeight(root) {
		if (!root) return -1;
	...
	isBalancedBST() {
		...
		return this.checkHeight(this) !== -Infinity ? this.getHeight() : -1;
	}
