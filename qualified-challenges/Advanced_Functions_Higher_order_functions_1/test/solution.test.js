...
describe("printNames", () => {
  ...
  it("uses forEach", () => {
     let spy = sinon.spy(names, 'forEach')
     printNames(names)
...
describe("logTreeType", () => {
  ...
  it("uses forEach", () => {
    let spy = sinon.spy(trees, 'forEach')
    logTreeType(trees)
...
describe("totalPoints", () => {
  ...
  it("uses forEach", () => {
    let spy = sinon.spy(points, 'forEach')
    totalPoints(points)
...
describe("buildSentence", () => {
  ...
  it("returns the full sentence", () => {
    expect(buildSentence(words)).equal("I'm looking for the man who shot my paw ")
  })
  
  it("uses forEach", () => {
    let spy = sinon.spy(words, 'forEach')
    buildSentence(words)
...
describe("logPercentages", () => {
  let decimals = [0.75,0.91,0.48,0.23,0.99,0.83,1.10]
  ...
  it("uses forEach", () => {
    let spy = sinon.spy(decimals, 'forEach')
    logPercentages(decimals)
